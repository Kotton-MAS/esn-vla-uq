...
`libero_spatial` を 1 タスク 1 試行 x 10 タスク収集 (成功率 90%)。
`scripts/collect_openpi_rollouts.py` で実際の `pi0_libero` policy server から取得した
ものです。

| スコア       | 被覆率        | 平均半幅 | 失敗検知 AUROC |
| ------------ | ------------- | -------- | -------------- |
| `absolute`   | 0.888 ± 0.042 | 2.90     | 0.500          |
| `normalized` | 0.881 ± 0.049 | **2.21** | 0.451          |

**被覆率は実データでも保たれています。** しかも保証の前提である交換可能性が成り立た
ない `across_task` split でこの値です。

**失敗検知は合成データの結果を再現せず、かつこのデータでは結論も出せません。**
10 エピソード中の失敗が 1 本しかなく、AUROC がその 1 本に乗っているためです。
ラベルも粗く (openpi には失敗開始時刻が無いのでエピソード全体を陽性とする)、
合成データの `failure_onset` ベースの数値とは比較できません。判断するには失敗を
増やした再収集が要ります。
