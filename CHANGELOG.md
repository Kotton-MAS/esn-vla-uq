...
- `calibrate` のレポートに、失敗検知が探索的な診断値である旨の注意書きを常に付ける
  ようにした。合成データで高い値 (0.87) が出るのは生成器の作りによるものである
...
- 失敗検知 AUROC が 0.5 を下回ったときに「信号が無いのではなく**向きが逆**」だと
  明示する注意書きを追加した。実 openpi ログでは 0.374 になるが、これは不具合では
  なく結果である (タイムアウト型の失敗ではチャンク分散が下がる)

### Documented

- `libero_10` 100 エピソードの検証結果を `docs/design.md` 10.10〜10.11 に記録した。
  **交換可能性の崩壊が実データで可視化された** (`across_task` の被覆率が 0.817 まで
  低下、ECE は `within_task` の 58 倍)。また**失敗検知の仮説が実データでは逆**
  だった (合成データは失敗でチャンク分散が上がるが、実データは下がる)

- 実 openpi ロールアウト 100 エピソードでの検証結果を `docs/design.md` 10.6〜10.8 に
  記録した。被覆率は名目どおり (within_task で 0.9033 ± 0.0102、ECE 0.0029) で、
  エピソード数を 10 倍にすると分散が約 1/5 になるという 9.3 節の予測が実データで
  確認できた

- **openpi からの収集は再現しない**ことを明記した。pi0 は flow matching で行動を
  サンプリングするため、同じ `--seed` でも軌道が変わる (実測: 2 回の収集で失敗した
  エピソードが違った)。収集済みログを入力にした解析は再現する
