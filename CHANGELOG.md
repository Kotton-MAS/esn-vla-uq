...
- **リザバー状態軌道の時間的特徴** (`diagnostics/trajectory.py`): 自己相関・実効次元
  (participation ratio)・新規性。いずれも過去だけを見る窓付きで、窓が埋まらない先頭は
  `NaN` を返す。タイムアウト失敗 (= 動作の反復) を時間方向から検知できるかの検証に
  使った。**検知に使えるとは主張しない** (14 節)

- `auroc_confidence_interval` (`calibration/metrics.py`): Hanley-McNeil の標準誤差に
  よる AUROC の信頼区間。**有効標本数はエピソード数を渡す** (ステップ数を渡すと区間が
  桁違いに狭く出る)。10.16 節の数値をこれで再現できるようになった

- `failure_mode_details` (`data/sources/openpi.py`): エピソード単位の失敗様式と落下
  開始時刻。`failure_mode_breakdown` の件数だけでは足りない解析のため
...
- **メモリ容量を診断対象とは別のリザバーで測っていた。** MC は `D_u=1` 固定で測って
  いたが、`Reservoir` は `W_in -> b -> W` の順に同一 RNG から引くため、同じ `seed`
  でも `n_inputs` が違えば `W` まで別物になる。`--n-inputs 17` のレポートには
  「17 次元入力のリザバーのスペクトル半径」と「別のリザバーのメモリ容量」が並んで
  いた。MC の定義が要求するのは駆動信号がスカラーであることなので、`D_u > 1` では
  `input_channel` の列にだけ信号を流して**同じリザバーで測る**。どの列で測ったかを
  レポートに残す。**`--n-inputs != 1` では測定値が変わる** (既定の 1 では変わらない)
...
- **診断指標は区間の質を予測しない。** `spectral_radius` × `leak_rate` の 15 点格子で
  メモリ容量を 6.4 倍動かしても幅は 5% しか動かず、105 対中 24 対しか有意差が出ない。
  リザバーの**有無**のほうが 2〜3 倍大きい。しかも相関の向きが逆で、MC が大きいほど
  区間は広い (Spearman +0.73)
- **`rho(A) = rho((1-a)I + aW)` は力学を要約しない。** `rho(A)` がほぼ同じでも MC は
  2〜3 倍違う。リーク率は状態更新式で入力の駆動強度そのものを掛けており、その経路は
  `rho(A)` に現れない。掃引の軸を `rho(A)` 1 本にまとめてはいけない
- **診断指標と幅の相関はスイートを跨ぐと符号が反転する。** libero_spatial では
  `rho(A)` の Spearman が −0.53 (libero_10 では +0.30)。片方だけ見て一般化していたら
  10.11 節と同じ誤りだった
- **失敗検知は「否定」ではなく「未決」で置く。** リザバー状態軌道の時間的特徴を
  事前登録した規則 (排除水準 0.6、有効標本数はエピソード数、符号も事前指定) で評価
  した。落下開始時刻に対しては 3 特徴とも 0.463〜0.487 で **0.6 を排除**する
  (デモ GIF が想定する「失敗直前に跳ねるバー」は支えられない)。一方エピソード単位の
  `dropped` / 新規性は 0.613 [0.533, 0.693] で**排除もできない**。標本を増やしても
  決着しない (0.6 の上下を分けるにはエピソード 1 万 6 千本規模が要る)
- **失敗様式によって時間的特徴の向きが逆になる。** `dropped` / `held_but_unplaced`
  では予測どおりだが `never_lifted` では逆 (掴めずに探索し続けるため)。混ぜると
  打ち消し合う。**符号を固定した検知器を作ってはいけない**
- **既定の `leak_rate=1.0` (非リーク型) は両スイートで最も悪いリーク率だった。**
  `a=0.5` で spatial の幅が平均 8.2% 縮む (5 点中 5 点で有意)。格子から得られた唯一の
  転移する知見であり、診断指標ではなくハイパーパラメータそのものだった。最適な `a` は
  スイートで違う (l10 は 0.2、spatial は 0.5) ため既定は変えない
