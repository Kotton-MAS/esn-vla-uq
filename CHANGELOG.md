...
- **遅延埋め込み baseline** (`calibrate --input-lags`、`SplitConformalPredictor` の
  `input_lags`)。read-out の設計行列に `u[t-1]..u[t-k]` を足す。`--readout input_only`
  と組み合わせると「ただの遅延線」になり、**リザバーが効かない理由が「タスクが記憶を
  要らない」のか「ESN の実装が下手」なのかを切り分けられる**
  (16 節)。ラグはエピソード境界を跨がず、区間の先頭は端点保持で埋める (行は落とさ
  ない。落とすと 12.3 節により残った行の非適合度が上がり、ラグ条件だけ不利になる)
...
- **失敗エピソードを含まないデータで `calibrate` の完了ログが消えていた。** AUROC が
  定義できないとき `mean_auroc` は `None` になるが、書式に `%.4f` を使っていたため
  logging が整形に失敗し、完了行の代わりに `--- Logging error ---` とスタックトレースが
  stderr に出ていた。終了コードとレポート JSON には影響しない。`demo` と同じく
  "n/a" と書くようにした
...
- **既定ハイパーパラメータを選び直した: `ridge_alpha` 1e-6 → 1.0、`leak_rate` 1.0 →
  0.7** (`docs/design.md` 15 節)。`leak_rate` × `ridge_alpha` の 25 通りを 4 データ
  セットで掃引し、**どのデータでも最良から最も離れない組**をミニマックスで選んだ。
  最悪比が 1.451 から 1.245 へ改善する。**単一のデータでの最良値ではない** ——
  どのデータでも良い組は存在せず、この組でもどこかで最良から 24% 離れる。
  実質的に効くのは `ridge_alpha` のほうで、`alpha=1` に固定すればリーク率をどこに
  置いても差は 6% に収まる。**選定に使っていない条件 (未使用の 4 コレクション + 別
  seed の合成データ、別の分割乱数種、`normalized` も含む 10 通り) で再検証した:
  有意に改善 6/10、有意に悪化 0/10、被覆率と ECE は不変** (15.6 節)。ただし改善は
  データに依存し、差が出ないコレクションもある
- **既定を使って生成した過去のレポート JSON の数値は再現しない。** どちらの既定で
  出したかは `esn_config` から判別できる。既定では `rho(A) != rho(W)` になった
  (以前は `leak_rate=1.0` で一致していた)
- 診断レポートのスキーマを 0.3.0 に、較正レポートのスキーマを 0.2.0 に上げた
  (`esn_config` への `use_reservoir` 追加と、`coverage` への幅の散らばり追加)
- README (英日) を 11〜16 節の結果に同期した。「リザバーは何を担っているか」節を
  新設し、スコープ節の失敗検知の記述を区間による決着と未決の内訳に差し替えた
...
- **診断指標と幅の相関はスイートを跨ぐと符号が反転する。** libero_spatial では
  ...
  10.11 節と同じ誤りだった
- **記憶に価値がある場面でも、ただの遅延線が ESN と同等以上だった。** 合成データでは
  遅延 1 本で幅が 4.4 倍縮むのに、リザバーは遅延線の 9 倍広く、記憶なしの `[1, u]`
  にすら負ける。libero_10 では遅延 5 本で 16.5% 改善し、やはりリザバーより狭い。
  libero_spatial では過去に価値が無く、どちらも `[1, u]` に負ける。**この課題設定
  では、区間幅を根拠にリザバーを正当化できない** (16 節)
- **必要な記憶は短い** (最良は `k=5` 付近、20 Hz なので 0.25 秒)。11.6 節の「`N` を
  増やすと効果が消える」は、記憶の中身ではなく read-out の表現力と推定分散の釣り合い
  だったことで説明が付いた
...
- **`leak_rate=1.0` (非リーク型) は、掃引した正則化強度の範囲では両スイートで最も悪い
  リーク率だった。** `a=0.5` で spatial の幅が平均 8.2% 縮む (5 点中 5 点で有意)
- **ただしその主張は `ridge_alpha` を跨いでは成り立たない。** `alpha=1` まで含めて
  掃引すると、spatial ではリーク率の効きの**向きが反転する** (`alpha=1` では `a=0.7`
  が 6.6% 良く、`alpha=100` では `a=1.0` が 5% 良い)。**2 つのハイパーパラメータは
  独立に効かない。片方だけを動かした掃引から向きを読んではいけない** (15.3 節で
  13.8 節を訂正した)
