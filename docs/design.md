...
**当初この表に「50」と書いたのは誤りだった。** `Pi0Config` のクラス既定値
（`action_horizon = 50`）を見て書いたが、`pi0_libero` は
`Pi0Config(action_horizon=10)` で上書きしている。実収集したログで 10 であることを
確認して訂正した。クラス既定値と学習設定の上書きを取り違えた例として残す。
...
### 10.6 失敗検知は実データで結論が出ていない

実 openpi の AUROC は 0.451 で、合成データの 0.871 とは対照的である。ただし
**収集した 10 エピソード中、失敗は 1 本しかない**。陽性はその 1 エピソードの
ステップだけであり、この数値から「実データでは検知できない」と結論するのは
統計的に無理がある。

またラベルの粒度も違う。openpi には失敗開始時刻が無いためエピソード単位の成否
（`episode_success`）へ落としており、合成データの `failure_onset` ベースの数値
とは直接比較できない。

**判断には失敗エピソードを増やした再収集が要る。** 成功率 90% なので、失敗を
10 本程度集めるには 1 タスクあたり 10 試行（計 100 エピソード）が目安になる。
