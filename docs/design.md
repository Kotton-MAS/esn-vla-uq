...
1. [目的・非目標](#1-%E7%9B%AE%E7%9A%84%E9%9D%9E%E7%9B%AE%E6%A8%99)
2. [アーキテクチャ](#2-%E3%82%A2%E3%83%BC%E3%82%AD%E3%83%86%E3%82%AF%E3%83%81%E3%83%A3)
3. [ESN の数学仕様](#3-esn-%E3%81%AE%E6%95%B0%E5%AD%A6%E4%BB%95%E6%A7%98)
4. [診断指標の定義](#4-%E8%A8%BA%E6%96%AD%E6%8C%87%E6%A8%99%E3%81%AE%E5%AE%9A%E7%BE%A9)
5. [データスキーマ v0.1](#5-%E3%83%87%E3%83%BC%E3%82%BF%E3%82%B9%E3%82%AD%E3%83%BC%E3%83%9E-v01)
6. [将来スプリントの決定事項](#6-%E5%B0%86%E6%9D%A5%E3%82%B9%E3%83%97%E3%83%AA%E3%83%B3%E3%83%88%E3%81%AE%E6%B1%BA%E5%AE%9A%E4%BA%8B%E9%A0%85)
7. [合成データの位置づけに関する誠実性宣言](#7-%E5%90%88%E6%88%90%E3%83%87%E3%83%BC%E3%82%BF%E3%81%AE%E4%BD%8D%E7%BD%AE%E3%81%A5%E3%81%91%E3%81%AB%E9%96%A2%E3%81%99%E3%82%8B%E8%AA%A0%E5%AE%9F%E6%80%A7%E5%AE%A3%E8%A8%80)
8. [未解決の設計論点（Sprint 2 持ち越し）](#8-%E6%9C%AA%E8%A7%A3%E6%B1%BA%E3%81%AE%E8%A8%AD%E8%A8%88%E8%AB%96%E7%82%B9sprint-2-%E6%8C%81%E3%81%A1%E8%B6%8A%E3%81%97)
9. [conformal 予測区間と較正評価（Sprint 2）](#9-conformal-%E4%BA%88%E6%B8%AC%E5%8C%BA%E9%96%93%E3%81%A8%E8%BC%83%E6%AD%A3%E8%A9%95%E4%BE%A1sprint-2)
...
1. **ESP 判定表（4.2 節）の妥当性の人間確認**: 既定設定（`ρ=0.9`）で十分条件
   ...
   （想定リスク 3）。
2. **openpi アダプタの実フィールドマッピング**: `state`（8 次元）/ `action`（7 次元）の
   次元・単位が openpi の policy server 実出力と一致するかは未検証。Sprint 2 で
   `OpenpiLogSource` 実装時に確定させる。
...
入力 `u[t] = [state[t], action[t]]`（`D_u = 15`）から目標 `y[t] = action[t+1]`
（`D_y = 7`）を予測する。各エピソードの最終ステップは目標が無いため落とし、
`T_i - 1` 標本になる。エピソード境界は跨がない（3.9 節）。

### 9.2 非適合度スコア: `absolute` は要件を満たさない

| スコア       | 区間幅           | 被覆率（20 分割平均） | 失敗検知 AUROC    |
| ------------ | ---------------- | --------------------- | ----------------- |
| `absolute`   | 全ステップで一定 | 0.895 ± 0.077         | **0.500 ± 0.000** |
| `normalized` | 入力ごとに変わる | 0.890 ± 0.085         | 0.612 ± 0.099     |

`absolute` の AUROC が 0.5 なのは偶然ではなく**定義上そうなる**。区間幅が定数
なら不確実性スコアも定数で、全ステップが同順位になる。被覆率は名目どおりでも、
「どのステップが危ないか」を一切区別しない。要件書のデモ GIF（失敗直前に
不確実性バーが跳ねる）は `normalized` でしか成立しないため、既定を
`normalized` にする。

`normalized` の `sigma(x)` は、リザバー状態から `log|r|` を予測する第 2 の ridge
read-out で推定する。**予測した `log sigma` は学習時に観測した範囲へ丸め込む。**
丸め込みが無いと、学習データの外側へわずかに外挿しただけで `exp` が発散し、
平均区間幅が行動の実スケール（0.01 程度）の 4000 倍（実測 44）になった。区間が
広いだけなら被覆率は上がるが、幅が意味を失い不確実性スコアとして使えなくなる。

`normalized` の平均区間幅は `absolute` の約 4 倍ある。同じ被覆率をより広い平均幅で
達成しているので、**平均幅の観点では `absolute` に劣る**。適応的な幅を得る対価と
して受け入れている。
